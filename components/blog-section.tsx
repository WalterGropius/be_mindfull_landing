        ...
        <div className="mt-16 grid gap-10 md:grid-cols-3 justify-items-center">
          {blogPosts.map((post) => (
            <div key={post.slug} className="flex flex-col w-full max-w-[388px] bg-surface-light rounded-[1rem] overflow-hidden">
              ...
              <div className="flex flex-col items-start flex-grow px-8 pt-8 pb-10 w-full">
                <Typography variant="h3" className="mb-4 text-2xl font-bold text-typography-heading leading-tight text-left">{post.title}</Typography>
                <Typography variant="p" className="mb-8 text-typography-body text-left">{post.excerpt}</Typography>
